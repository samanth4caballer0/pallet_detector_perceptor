...
std::vector<ReflectorDetection> ReflectorDetector::detect(const double & __angle_init, const double & __angle_end, const std::vector<float> & __ranges, const std::vector<float> & __intensities, const std::vector<uint8_t> & __reflector_hits, const Eigen::Isometry2d & __T_platform_sensor)
{
	...
	// check empty conditions
	if ( __ranges.empty() || __intensities.empty() || __reflector_hits.empty() || (__ranges.size() != __intensities.size()) ||  (__ranges.size() != __reflector_hits.size()) )
		return detections;
	...
	double corrected_range;
	Eigen::Vector2d point_in_lidar;
	for ( unsigned int i = 0; i < __reflector_hits.size(); i++ )
	{
		if ( ( __reflector_hits[i] > 0 ) && ( __ranges[i] < max_detection_range__ ) )
		{
			// From polar to cartesian coordinates in sensor frame
			azimuth = __angle_init + i*angle_increment;
			corrected_range = correctDistortion(__ranges[i]);
